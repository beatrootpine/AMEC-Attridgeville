...
export function useAdmin() {
  ...
  useEffect(() => {
    ...
    authClient.auth.getSession().then(async ({ data }) => {
      ...
      try {
        const { data: admin } = await authClient.from('admins').select('id').eq('user_id', user.id).maybeSingle()
...
export async function adminSignIn(email, password) {
  ...
  // Check admin
  const { data: admin } = await authClient.from('admins').select('id').eq('user_id', data.user.id).maybeSingle()
  if (!admin) {
